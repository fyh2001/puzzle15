<template>
  <div>
    <top-bar title="最佳步数详情" class="py-4 ml-2">
      <template #right>
        <div>
    ...
    <div class="p-4">
      ...
      <transition name="fade" mode="out-in">
        <n-el
          class="py-10 w-full bg-white rounded-2xl shadow bg-cover overflow-hidden"
          ...
          <div class="grid grid-cols-2 gap-y-4 mt-4 text-center">
            ...
            <div>
              ...
              <div
                ...
                :class="[
                  {
                    'text-red-6':
                      formatDurationInRecord(recordDetail.duration) < 10,
                  },
                  {
                    'text-green-6':
                      formatDurationInRecord(recordDetail.duration) > 10 &&
                      formatDurationInRecord(recordDetail.duration) < 20,
                  },
                  {
                    'text-indigo-6':
                      formatDurationInRecord(recordDetail.duration) > 20 &&
                      formatDurationInRecord(recordDetail.duration) < 30,
                  },
                ]"
              >
                {{ formatDurationInRecord(recordDetail.duration) }}
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div class="text-4.5 font-bold">
                {{ formatDataByTimestamp(parseInt(recordDetail.updatedAt)) }}
              </div>
            </div>
          </div>
        </n-el>
...
const getRecordDetail = async () => {
  const { data } = await recordRequest.getById(
    recordStore.recordDetailParams.bestStepCountId
  );
